package io.github.qishr.cascara.lang.jsonpath.processor;

public class EvaluationContext {
    public final Object root;     // $
    public final Object current;  // @
    public final Object parent;   // optional
    public final Integer index;   // optional

    public EvaluationContext(Object root, Object current, Object parent, Integer index) {
        this.root = root;
        this.current = current;
        this.parent = parent;
        this.index = index;
    }

    public EvaluationContext withCurrent(Object newCurrent) {
        return new EvaluationContext(root, newCurrent, current, index);
    }

    public EvaluationContext withIndex(int idx) {
        return new EvaluationContext(root, current, parent, idx);
    }
}
